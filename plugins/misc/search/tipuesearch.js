...
function getTipueSearch(start, replace)
{
     ...
     if (d.length >= tipuesearch_minimum_length)
     {
          ...
          if (c != 0)
          {
               ...
               if (c == 1)
               {
                    out += '<div id="tipue_search_results_count">1条结果</div>';
               }
               else
               {
                    c_c = c.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
                    out += '<div id="tipue_search_results_count">' + c_c + ' 条结果</div>';
               }
          ...
          {
               out += '<div id="tipue_search_warning_head">没找到相关内容</div>'; 
          }          
     }
     else
     {
          ...
          {
               out += '<div id="tipue_search_warning_head">关键词太短</div>';
               if (tipuesearch_minimum_length == 1)
               {
                    out += '<div id="tipue_search_warning">至少为两个字符</div>';
               }
               else
               {
                    out += '<div id="tipue_search_warning">至少为 ' + tipuesearch_minimum_length + ' 个字符</div>';
               }
          }
     }
